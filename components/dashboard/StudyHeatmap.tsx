...
export function StudyHeatmap({ sessions }: StudyHeatmapProps) {
  ...
  const getColor = (durationSeconds: number) => {
    if (durationSeconds === 0) return 'rgba(255,255,255,0.03)'
    const hours = durationSeconds / 3600
    if (hours < 1) return 'rgba(132, 204, 22, 0.3)'   // light neon
    if (hours < 3) return 'rgba(132, 204, 22, 0.5)'   // medium
    if (hours < 5) return 'rgba(132, 204, 22, 0.8)'   // strong
    return 'rgba(132, 204, 22, 1)'                    // full neon
  }
  ...
  return (
    <div className="glass-card" style={{ padding: '24px', overflowX: 'auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
        <h3 style={{ fontSize: '15px', fontWeight: 700, color: '#f9fafb' }}>Heatmap de Estudo Anual</h3>
        <div style={{ display: 'flex', gap: '4px', alignItems: 'center', fontSize: '11px', color: '#6b7280' }}>
          <span>Menos</span>
      ...
      <div style={{ display: 'flex', gap: '4px', minWidth: '800px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', paddingRight: '8px', paddingTop: '16px' }}>
          <span style={{ fontSize: '10px', color: '#6b7280', height: '12px' }}></span>
          <span style={{ fontSize: '10px', color: '#6b7280', height: '12px', lineHeight: '12px' }}>Seg</span>
          <span style={{ fontSize: '10px', color: '#6b7280', height: '12px' }}></span>
          <span style={{ fontSize: '10px', color: '#6b7280', height: '12px', lineHeight: '12px' }}>Qua</span>
          <span style={{ fontSize: '10px', color: '#6b7280', height: '12px' }}></span>
          <span style={{ fontSize: '10px', color: '#6b7280', height: '12px', lineHeight: '12px' }}>Sex</span>
          <span style={{ fontSize: '10px', color: '#6b7280', height: '12px' }}></span>
        </div>
        ...
        <div style={{ display: 'flex', gap: '4px', flex: 1 }}>
          {weeks.map((week, i) => (
            <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              ...
              <div style={{ height: '12px', fontSize: '10px', color: '#6b7280' }}>
