  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      <div style={{ marginBottom: '28px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: 800, color: '#f9fafb', marginBottom: '6px' }}>
            Registrar Estudo
          </h1>
          <p style={{ color: '#9ca3af', fontSize: '14px' }}>Registre sua sessão de estudos e acompanhe seu progresso</p>
        </div>
      ...
      <div className="grid-cols-responsive-main">
        <div className="glass-card" style={{ padding: '28px' }}>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            {/* Date */}
            <div>
              <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: '#d1d5db', marginBottom: '8px' }}>Data</label>
              <div style={{ display: 'flex', gap: '8px' }}>
                <input type="date" className="input-glass" value={date} onChange={e => setDate(e.target.value)} style={{ flex: 1 }} />
                <button type="button" onClick={() => setDate(new Date().toISOString().split('T')[0])} style={{ padding: '10px 16px', background: 'rgba(132,204,22,0.1)', border: '1px solid rgba(132,204,22,0.2)', borderRadius: '10px', color: '#84cc16', fontSize: '13px', fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap', transition: 'all 0.2s' }}>
                  Hoje
                </button>
              </div>
            </div>
            ...
            {/* Subject */}
            <div>
              <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: '#d1d5db', marginBottom: '8px' }}>Matéria</label>
            ...
            {/* Duration */}
            <div>
              <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: '#d1d5db', marginBottom: '8px' }}>Duração do Estudo</label>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
                {[
                  ...
                  <div key={label}>
                    <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '4px', textAlign: 'center' }}>{label}</div>
              ...
              {totalSeconds > 0 && (
                <div style={{ marginTop: '10px', display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 14px', background: 'rgba(132,204,22,0.08)', borderRadius: '8px', border: '1px solid rgba(132,204,22,0.15)' }}>
                  <Clock size={14} color="#84cc16" />
                  <span style={{ fontSize: '13px', color: '#84cc16', fontWeight: 600 }}>Total: {totalHours}h ({totalSeconds}s)</span>
                </div>
              )}
            </div>
            ...
            {/* Questions */}
            <div>
              <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: '#d1d5db', marginBottom: '8px' }}>Questões (opcional)</label>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                <div>
                  <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '4px' }}>Respondidas</div>
                ...
                <div>
                  <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '4px' }}>Acertos</div>
              ...
              {questions > 0 && (
                <div style={{ marginTop: '8px', fontSize: '12px', color: '#9ca3af' }}>Taxa de acerto: <span style={{ color: acerto >= 70 ? '#84cc16' : '#f59e0b', fontWeight: 600 }}>{acerto}%</span></div>
              )}
            </div>

            {/* Note */}
            <div>
              <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: '#d1d5db', marginBottom: '8px' }}>Anotações (opcional)</label>
        ...
        <div>
          <div className="glass-card" style={{ padding: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
              <CalendarDays size={16} color="#84cc16" />
              <h3 style={{ fontSize: '15px', fontWeight: 700, color: '#f9fafb' }}>Sessões desta semana</h3>
            </div>
            {recentSessions.length === 0 ? (
              <p style={{ color: '#6b7280', fontSize: '13px' }}>Nenhuma sessão registrada esta semana.</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {recentSessions.slice(0, 8).map((s: any) => (
                  <motion.div whileHover={{ x: 4 }} key={s.id} style={{ padding: '12px 14px', background: 'rgba(255,255,255,0.03)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      ...
                      <div>
                        <div style={{ fontSize: '13px', fontWeight: 600, color: '#f9fafb' }}>{s.subject?.name}</div>
                        <div style={{ fontSize: '11px', color: '#6b7280' }}>{new Date(s.date).toLocaleDateString('pt-BR')}</div>
                      </div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: '13px', fontWeight: 700, color: '#84cc16' }}>{(s.duration / 3600).toFixed(1)}h</div>
                      {s.questions > 0 && <div style={{ fontSize: '11px', color: '#9ca3af' }}>{s.questions}Q</div>}
                    </div>
                  </motion.div>
                ))}
              </div>
